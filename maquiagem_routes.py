...
class Maquiagem_Routes:
    
    # Função para buscar os produtos mais vendidos
    def obter_produtos_maquiagem_mais_vendidos():
        db = conectar_db()
        cursor = db.cursor()
        ...
        WHERE p.tipo_produto = 'Maquiagem'
        GROUP BY p.id
    ...
    def obter_tipos_maquiagem():
        db = conectar_db()
        cursor = db.cursor()

        query = "SELECT id, nome FROM tipo_produtos WHERE categoria = 'Maquiagem'"
        cursor.execute(query)

$(document).ready(function() {
    ...
    $(document).on("keyup", ".produto", function() {
        ...
        $.ajax({
            url: "/buscar-produtos-update-admin",
            method: "POST",
            data: { q: termo },
            success: function(response) {
                ...
                if (response.length > 0) {
                    ...
                    response.forEach(produto => {
                        select.append(
                            `<option value="${produto.id}" data-qtd-comprada="${produto.qtd_comprada}">${produto.nome} (Quantidade: ${produto.qtd_comprada})</option>`
                        );
                    });
    ...
    $(document).on("change", ".sugestao-select", function() {
        ...
        let qtdComprada = $(this).find("option:selected").data("qtd-comprada");

        $(this).closest(".produto-item").find(".produto").val(nomeProduto);
        $(this).closest(".produto-item").find(".produto_id").val(produtoId);
        $(this).closest(".produto-item").find(".produto_qtd").val(qtdComprada); 
